    ...
    <main class="container" style="max-width:700px; margin-top:40px;">
            ...
            <table class="table table-bordered" style="width:100%;">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div class="w-24 h-24 overflow-hidden flex items-center justify-center bg-gray-100"
                            style="border-radius:5px;">
                            <img id="preview_hinh_anh" src="../uploads/<?php echo htmlspecialchars($hinh_anh); ?>"
                                alt="Ảnh sản phẩm" class="object-contain h-full">
                        </div>
        ...
        if (isset($_POST['btnSave'])) {
            ...
            if (!empty($_FILES["hinh_anh"]["name"])) {
                $img_name = time() . "_" . basename($_FILES["hinh_anh"]["name"]); // tránh trùng tên
                ...
                if (in_array($imageFileType, $allowed_types)) {
                    if (move_uploaded_file($_FILES["hinh_anh"]["tmp_name"], $target_file)) {
                        ...
                        echo "<script>alert(' Không thể upload ảnh. Vui lòng thử lại.');</script>";
                    }
                } else {
                    echo "<script>alert(' Chỉ được upload file ảnh (JPG, PNG, GIF, WEBP)!');</script>";
                }
}
            ...
            if ($edit_mode) {
                $sql = "UPDATE san_pham SET 
                        ten_san_pham=:ten_san_pham, gia=:gia, giam_phan_tram=:giam_phan_tram, mo_ta=:mo_ta, hinh_anh=:hinh_anh,
                        id_danh_muc=:id_danh_muc, tinh_trang=:tinh_trang, noi_bat=:noi_bat, moi=:moi
                        WHERE id=:id";
                $stmt = $conn->prepare($sql);
                $stmt->bindValue(':id', $id, PDO::PARAM_INT);
            } else {
            ...
            if ($stmt->execute()) {
                echo "<script>alert(' Lưu sản phẩm thành công!'); window.location='list_sanpham.php';</script>";
            } else {
                echo "<script>alert(' Lỗi khi lưu sản phẩm!');</script>";
            }
        }
        ?>
    </main>
